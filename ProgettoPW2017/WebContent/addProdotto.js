/**
 * Aggiunge un prodotto con tutte le sue caratteristiche alla schermata che al
 * momento si sta visualizzando (lì dove è presente il div con id "showing").
 * 
 * @param prod
 *            Prodotto che stiamo per aggiungere con tutte le sue informazioni
 * @param q
 *            Valore utilizzato per creare un id dinamico della tabella che
 *            stiamo per costruire contente il prodotto
 * @param val
 *            Valore utilizzato in modo binario, che identifica se stiamo
 *            parlando di un utente normale (0), oppure di un amministratore (1)
 *            In base a val si avranno dei layout diversi che permetteranno ai
 *            due attori di interagire diversamente col sistema
 */
function addProdotto(prod, q, val)
{
	if ((prod.disp < 1) && (val == 0)){;} else
	{
		var tab, tr, td, div;
		div = document.getElementById("showing");// div per indicare da dove
		// inserire gli elementi
		tab = document.createElement("table"); // creo la tabella per ogni singolo
		// elemento
		tab.setAttribute("class", "showProduct"); // tutte le tabelle avranno la
		// stessa classe per modificarle
		// in header.css
		...
		// visualizzerà il nome del prodotto
		th.setAttribute("colspan", 2);// rendo la riga larga come due colonnne
		th.appendChild(document.createTextNode(prod.nome)); // creo un nodo che
		// inserisco nell'header
		// col nome del prodotto
		...
		var center = document.createElement("center"); // creo un elemento html5
		// per intermediare con l
		// immagine
		var img = document.createElement("img"); // creo l immagine
		img.src = "Imm/" + prod.nomeImg; // assegno all immagine la sua giusta
		// sorgente
		td = document.createElement("td");// creo il table data dove mettere il
		// tutto
		...
		center.appendChild(img); // all involucro dell immagine concateno
		...
		td = document.createElement("td"); // creo l elemento dove effettivamete
		// mettero la label per poi mettere il
		// codice
		...
		if (val)
		{
			var codic = document.createElement("input"); // creo un altro td per
			// il codice vero e
			// proprio
		...
		{
			td = document.createElement("td"); // creo un altro td per il codice
			// vero e proprio
			td.appendChild(document.createTextNode(prod.codice));// inserisco il
			// codice
		...
		if (val)
		{
			var color = document.createElement("input"); // creo un altro td per
			// il codice vero e
			// proprio
		...
		{
			td = document.createElement("td"); // creo un altro td per il colore
			// vero e proprio
		...
		if (val)
		{
			var disponibilit = document.createElement("input"); // creo un altro td
			// per il codice
			// vero e proprio
		...
		if (val)
		{
			var sess = document.createElement("input"); // creo un altro td per il
		...
		tr = document.createElement("tr"); // creo la riga per la quantità del
		// prodotto che voglio selezionare e per
		// il carello
		...
		if (val)
		{
			var quer = "?codice=" + prod.codice + "&colore=" + prod.prodCol
					+ "&taglia=" + prod.prodTag;
			...
			input1.onclick = function()
			{
				...
				xhtt.open("GET", "Modifica" + quer, true);
		...
		{
			var input = document.createElement("input"); // creo l elemento per l
			// input
			input.setAttribute("type", "number"); // non tutti i broswer lo
			// prendono (devo
			// ridimensionarlo)
			input.setAttribute("placeholder", "Quantità ord."); // suggerisco a cosa
			// serve questo
			// campo
			var x = "Quanti" + q; // assegno un id dinamico a questo elemento,
			// preso dal for
			input.setAttribute("class", "elemento");
			input.setAttribute("id", x); // in modo da prendermi il valore al suo
			// interno quando necessario
			// ////alert(input.getAttribute("id"));
			td.appendChild(center); // incerisco l involucro dell input
			center.appendChild(input); // inserisco l input nell involucro
			tr.appendChild(td); // inserisco il tutto nella riga
		}
		// CARRELLO. Inserimento ed implementazione del bottone come carrello. O di
		// elimina per amministratore
		var btn = document.createElement("input");
		btn.setAttribute("type", "image");
		if (val)// essendo che val sarà 0 per amministratore, e 1 per utenti
		// normali, sono invertiti
		{
			btn.src = "Imm/x.png";
			...
			btn.onclick = function()
			{
				...
				div.removeChild(tab);
				valid.splice(q - 1, 1);// perchè q parte da 1 mentre l array parte
				// da 0
				// splice si usa sia per aggiungere che per eliminare, nell ultimo
				// caso il secondo parametro è settato a 1
				// ////alert(q);// ricordando che q parte da 0
				// ////alert(valid);
				// document.getElementById("ciao").innerHTML= prod.codice +" "+ " "
				// +prod.prodCol+ " " + prod.prodTag +" "+ numeroEle;// devi
				// inserire il p con id ciao per vederlo
				var http = new XMLHttpRequest();
				var query = "?codice=" + prod.codice + "&colore=" + prod.prodCol
						+ "&taglia=" + prod.prodTag;
				http.open("GET", "DeleteAmm" + query, true);
				// //alert("sto inviando"+ query);
				http.send();
				alert("Prodotto " + prod.codice + " eliminato");
			};
		} else
		{
			btn.src = "Imm/carrello.png";
			btn.title = "Aggiungi al carrello";
			// associo al click del bottone una funzione che semplicemnte mi prende
			// i parametri in cui esso viene cliccato
			btn.onclick = function()
			{
				// ////alert(tab.getAttribute("id"));
				...
				} else
				{
					...
					} else
					{
						...
						// valid.splice(q-1,1);//perchè q parte da 1 mentre l array
						// parte da 0
						// splice si usa sia per aggiungere che per eliminare, nell
						// ultimo caso il secondo parametro è settato a 1
						// ////alert(q);// ricordando che q parte da 0
						// ////alert(valid);
						// document.getElementById("ciao").innerHTML= prod.codice +"
						// "+ " " +prod.prodCol+ " " + prod.prodTag +" "+
						// numeroEle;// devi inserire il p con id ciao per vederlo
						...
						xhttp.onreadystatechange = function()
						{
							...
							{
								// //alert ("sto ricevendo la seguente risposta "+
								// this.responseText); // ricevo un oggetto JSON
								obj = JSON.parse(this.responseText);
								console.log(this.responseText);
								console.log(obj.prodotti.lenght);
								// //alert("oggetto javascript"+ obj.prodotti); //
								// ho un oggetto
							}
						};
						var query = "?codice=" + prod.codice + "&colore="
								+ prod.prodCol + "&taglia=" + prod.prodTag
								+ "&numElementi=" + numeroEle;// DA INSERIRE # ELE
						// DA INPUT
		...
		tab.appendChild(tr);// aggiungo la riga con l involucro all interno della
		// tabella
